...
class class_cls {
        ...
        public static function is_controller( $current_user = false ){
        ...
        public static function can( $array ){
                global $current_user;
                $user_id = (int) $current_user->ID;
                $role = $user_id > 0 ? $current_user->roles[0] : 'guest';
                ...
                return !self::is_controller() ? in_array($user_id, $array) || in_array( $role, $array ) : true;
        }
        ...
        public static function get_users( $user_role = '' ){
                global $wpdb;

                $roles = self::get_roles();
                $users = array();
                $total = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->users}");
                $cls_settings = (array) get_option( 'cls_settings', array( 'controller' => get_current_user_ID() ) );
                $controller = $cls_settings['controller'];
