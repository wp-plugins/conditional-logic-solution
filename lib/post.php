...
class cls_posts extends class_cls {
        ...
        public function is_post( $wp_query = '' ){
                ...
                if( empty( $post_type ) && isset( $wp_query->query_vars['post_type'] ) ) $post_type = $wp_query->query_vars['post_type'];
        ...
        public function init(){
                        ...
                        self::$_values = $this->values = $values = (object) $this->values();
                        $this->usercaps = isset( $this->values->usercaps ) ? (array) $this->values->usercaps : array();
        ...
        public static function can( $cap ){
                global $current_user;
                $caps = isset( self::$_values->caps ) ? (array) self::$_values->caps : array();
                $role = is_object( $current_user ) && $current_user->ID > 0  ? $current_user->roles[0] : 'guest';
                
                if( $cap == 'read' ){
                        $reader = (array) self::$_values->read;
        ...
        public function save_post($post_id){
                ...
                if( isset( $_POST['post_type'] ) && !in_array($_POST['post_type'], (array) $this->post_type) ) return;
                
                $nonce = isset( $_REQUEST['cls_post_nonce'] ) ? $_REQUEST['cls_post_nonce'] : '';
                
                if( !empty($nonce) && wp_verify_nonce( $nonce, "cls_{$this->post_type}" ) ){
                        
                        $usercaps = isset( $_REQUEST['user_caps'] ) ? $_REQUEST['user_caps'] : '';
                        update_post_meta( $post_id, 'user_caps', $usercaps);
                        
                        $old_usercaps = isset( $this->values->usercaps ) ? (array) $this->values->usercaps : array();
        ...
        public function get_post_usercaps( $cap = 'read' ){
                ...
                $role = $user_id > 0 ? $current_user->roles[0] : 'guest';
                $ids = array();
